...
mode7= zeros(N_sample,N+1);
mode8= zeros(N_sample,N+1);
mode9= zeros(N_sample,N+1);
mode10= zeros(N_sample,N+1);
mode11= zeros(N_sample,N+1);
mode12= zeros(N_sample,N+1);
mode13= zeros(N_sample,N+1);
mode14= zeros(N_sample,N+1);
mode15= zeros(N_sample,N+1);
datasets = cell(2,2);
...
for num_sample=1:N_sample    
    ...
    y=0.5*(audio(Ac,fs,N)+jam_zaidai(Ac,fs,N));
    yr=awgn(y,snr,'measured','db');
    mode6(num_sample,:)=[6,yr]; 
    y=0.5*(audio(Ac,fs,N)+LFM(Ac,fs,N));
    yr=awgn(y,snr,'measured','db');
    mode7(num_sample,:)=[7,yr];     
    y=0.5*(audio(Ac,fs,N)+ssc(Ac,fs,fc,N));
    yr=awgn(y,snr,'measured','db');
    mode8(num_sample,:)=[8,yr];   
    y=0.5*(audio(Ac,fs,N)+jam_pulse(Ac,fs,fc,N));
    yr=awgn(y,snr,'measured','db');
    mode9(num_sample,:)=[9,yr];
    y=0.5*(LFM(Ac,fs,N)+jam_zaidai(Ac,fs,N));
    yr=awgn(y,snr,'measured','db');
    mode10(num_sample,:)=[10,yr];
    y=0.5*(ssc(Ac,fs,fc,N)+jam_zaidai(Ac,fs,N));
    yr=awgn(y,snr,'measured','db');
    mode11(num_sample,:)=[11,yr];
    y=0.5*(jam_pulse(Ac,fs,fc,N)+jam_zaidai(Ac,fs,N));
    yr=awgn(y,snr,'measured','db');
    mode12(num_sample,:)=[12,yr];
    y=0.5*(LFM(Ac,fs,N)+ssc(Ac,fs,fc,N));
    yr=awgn(y,snr,'measured','db');
    mode13(num_sample,:)=[13,yr];
    y=0.5*(LFM(Ac,fs,N)+jam_pulse(Ac,fs,fc,N));
    yr=awgn(y,snr,'measured','db');
    mode14(num_sample,:)=[14,yr];
    y=0.5*(ssc(Ac,fs,fc,N)+jam_pulse(Ac,fs,fc,N));
    yr=awgn(y,snr,'measured','db');
