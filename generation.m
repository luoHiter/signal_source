...
for num_sample=1:N_sample    
    ...
    y=audio(Ac,fs,N)+jam_zaidai(Ac,fs,N);
    yr=awgn(y,snr,'measured','db');
    mode6(num_sample,:)=[6,yr]; 
    y=audio(Ac,fs,N)+LFM(Ac,fs,N);
    yr=awgn(y,snr,'measured','db');
    mode7(num_sample,:)=[7,yr];     
    y=audio(Ac,fs,N)+ssc(Ac,fs,fc,N);
    yr=awgn(y,snr,'measured','db');
    mode8(num_sample,:)=[8,yr];   
    y=audio(Ac,fs,N)+jam_pulse(Ac,fs,fc,N);
    yr=awgn(y,snr,'measured','db');
    mode9(num_sample,:)=[9,yr];
    y=LFM(Ac,fs,N)+jam_zaidai(Ac,fs,N);
    yr=awgn(y,snr,'measured','db');
    mode10(num_sample,:)=[10,yr];
    y=ssc(Ac,fs,fc,N)+jam_zaidai(Ac,fs,N);
    yr=awgn(y,snr,'measured','db');
    mode11(num_sample,:)=[11,yr];
    y=jam_pulse(Ac,fs,fc,N)+jam_zaidai(Ac,fs,N);
    yr=awgn(y,snr,'measured','db');
    mode12(num_sample,:)=[12,yr];
    y=LFM(Ac,fs,N)+ssc(Ac,fs,fc,N);
    yr=awgn(y,snr,'measured','db');
    mode13(num_sample,:)=[13,yr];
    y=LFM(Ac,fs,N)+jam_pulse(Ac,fs,fc,N);
    yr=awgn(y,snr,'measured','db');
    mode14(num_sample,:)=[14,yr];
    y=ssc(Ac,fs,fc,N)+jam_pulse(Ac,fs,fc,N);
    yr=awgn(y,snr,'measured','db');
